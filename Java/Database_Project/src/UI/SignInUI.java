...
private boolean verifyCredentials(String username, String password) {
    ...
    try (Connection connection = DBConnection.getConnection();
         PreparedStatement vcstmt = connection.prepareStatement(sql)) {
        vcstmt.setString(1, username);
        vcstmt.setString(2, password);

        try (ResultSet rs = vcstmt.executeQuery()) {
            if (rs.next()) {
            String aUser = rs.getString("Username");

            SessionStore.setLoggedInUser(aUser);;
            // Create User object and save information
            newUser = new User(aUser); // Assuming User constructor takes these parameters
            return true;
            }
        }
